...
class Green_Enemy(pygame.sprite.Sprite):
    def __init__(self):
    ...
    def update(self, bullets):
        ...
        if (self.frame < 5) :
            ...
            self.surf = pygame.image.load(Paths().select_sprite("green_2.png")).convert_alpha()
            self.size = self.surf.get_size()
            self.image = pygame.transform.scale(self.surf, (int(self.size[0]*2), int(self.size[1]*2)))
            self.frame += 1
        elif (self.frame < 15) :
            ...
            self.surf = pygame.image.load(Paths().select_sprite("green_3.png")).convert_alpha()
            self.size = self.surf.get_size()
            self.image = pygame.transform.scale(self.surf, (int(self.size[0]*2), int(self.size[1]*2)))
            self.frame += 1
        else :
